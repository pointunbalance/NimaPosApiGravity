---
name: "purchases"
description: "Guidelines and functional requirements for المشتريات والموردين"
---

# المشتريات والموردين

هذا الملف (Skill) يوفر معايير هندسية متكاملة لبناء وتوسيع قسم **المشتريات والموردين** داخل التطبيق، معتمداً على بيئة العمل الحالية (React + Dexie.js + TypeScript) لضمان العمل بدون اتصال بالإنترنت (Offline-First).
...
- **سجل العمليات (Audit Logging)**: توثيق الحركات الحساسة (إضافة، تعديل، حذف) عبر دالة `logActivity()` كما في هذا المثال:
  ...
  await logActivity('purchases', 'عملية تصنيف', 'تم تسجيل إجراء ضمن القسم بنجاح');
  ```
