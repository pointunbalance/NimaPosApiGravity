---
name: "studio"
description: "Guidelines and functional requirements for قسم الاستوديو والتصوير"
---

# قسم الاستوديو والتصوير

هذا الملف (Skill) يوفر معايير هندسية متكاملة لبناء وتوسيع قسم **قسم الاستوديو والتصوير** داخل التطبيق، معتمداً على بيئة العمل الحالية (React + Dexie.js + TypeScript) لضمان العمل بدون اتصال بالإنترنت (Offline-First).
...
- **سجل العمليات (Audit Logging)**: توثيق الحركات الحساسة (إضافة، تعديل، حذف) عبر دالة `logActivity()` كما في هذا المثال:
  ...
  await logActivity('studio', 'عملية تصنيف', 'تم تسجيل إجراء ضمن القسم بنجاح');
  ```
