---
name: "manufacturing"
description: "Guidelines and functional requirements for التصنيع والإنتاج"
---

# التصنيع والإنتاج

هذا الملف (Skill) يوفر معايير هندسية متكاملة لبناء وتوسيع قسم **التصنيع والإنتاج** داخل التطبيق، معتمداً على بيئة العمل الحالية (React + Dexie.js + TypeScript) لضمان العمل بدون اتصال بالإنترنت (Offline-First).
...
- **سجل العمليات (Audit Logging)**: توثيق الحركات الحساسة (إضافة، تعديل، حذف) عبر دالة `logActivity()` كما في هذا المثال:
  ...
  await logActivity('manufacturing', 'عملية تصنيف', 'تم تسجيل إجراء ضمن القسم بنجاح');
  ```
