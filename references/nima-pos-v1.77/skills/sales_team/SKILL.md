---
name: "sales_team"
description: "Guidelines and functional requirements for فريق المبيعات والتوزيع"
---

# فريق المبيعات والتوزيع

هذا الملف (Skill) يوفر معايير هندسية متكاملة لبناء وتوسيع قسم **فريق المبيعات والتوزيع** داخل التطبيق، معتمداً على بيئة العمل الحالية (React + Dexie.js + TypeScript) لضمان العمل بدون اتصال بالإنترنت (Offline-First).
...
- **سجل العمليات (Audit Logging)**: توثيق الحركات الحساسة (إضافة، تعديل، حذف) عبر دالة `logActivity()` كما في هذا المثال:
  ...
  await logActivity('sales_team', 'عملية تصنيف', 'تم تسجيل إجراء ضمن القسم بنجاح');
  ```
