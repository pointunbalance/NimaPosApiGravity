...
description: "Guidelines and functional requirements for نظام التذاكر"
---

# نظام التذاكر

هذا الملف (Skill) يوفر معايير هندسية متكاملة لبناء وتوسيع قسم **نظام التذاكر** داخل التطبيق، معتمداً على بيئة العمل الحالية (React + Dexie.js + TypeScript) لضمان العمل بدون اتصال بالإنترنت (Offline-First).
...
- **سجل العمليات (Audit Logging)**: توثيق الحركات الحساسة (إضافة، تعديل، حذف) عبر دالة `logActivity()` كما في هذا المثال:
  ...
  await logActivity('ticket_bookings', 'عملية تصنيف', 'تم تسجيل إجراء ضمن القسم بنجاح');
  ```
