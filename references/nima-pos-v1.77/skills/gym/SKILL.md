---
name: "gym"
description: "Guidelines and functional requirements for الأندية الرياضية (الجيم)"
---

# الأندية الرياضية (الجيم)

هذا الملف (Skill) يوفر معايير هندسية متكاملة لبناء وتوسيع قسم **الأندية الرياضية (الجيم)** داخل التطبيق، معتمداً على بيئة العمل الحالية (React + Dexie.js + TypeScript) لضمان العمل بدون اتصال بالإنترنت (Offline-First).
...
- **لوحة قيادة الجيم** (/gym/dashboard)
...
- **سجل العمليات (Audit Logging)**: توثيق الحركات الحساسة (إضافة، تعديل، حذف) عبر دالة `logActivity()` كما في هذا المثال:
  ...
  await logActivity('gym', 'عملية تصنيف', 'تم تسجيل إجراء ضمن القسم بنجاح');
  ```
