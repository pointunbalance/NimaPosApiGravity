---
name: "education"
description: "Guidelines and functional requirements for نظام الحضانة وروضة الأطفال"
---

# نظام الحضانة وروضة الأطفال

هذا الملف (Skill) يوفر معايير هندسية متكاملة لبناء وتوسيع قسم **نظام الحضانة وروضة الأطفال** داخل التطبيق، معتمداً على بيئة العمل الحالية (React + Dexie.js + TypeScript) لضمان العمل بدون اتصال بالإنترنت (Offline-First).
...
- **سجل العمليات (Audit Logging)**: توثيق الحركات الحساسة (إضافة، تعديل، حذف) عبر دالة `logActivity()` كما في هذا المثال:
  ...
  await logActivity('education', 'عملية تصنيف', 'تم تسجيل إجراء ضمن القسم بنجاح');
  ```
