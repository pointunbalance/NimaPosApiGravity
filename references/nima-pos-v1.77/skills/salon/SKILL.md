---
name: "salon"
description: "Guidelines and functional requirements for صوالين التجميل والحلاقة"
---

# صوالين التجميل والحلاقة

هذا الملف (Skill) يوفر معايير هندسية متكاملة لبناء وتوسيع قسم **صوالين التجميل والحلاقة** داخل التطبيق، معتمداً على بيئة العمل الحالية (React + Dexie.js + TypeScript) لضمان العمل بدون اتصال بالإنترنت (Offline-First).
...
- **لوحة قيادة الصالون** (/salon/dashboard)
...
- **سجل العمليات (Audit Logging)**: توثيق الحركات الحساسة (إضافة، تعديل، حذف) عبر دالة `logActivity()` كما في هذا المثال:
  ...
  await logActivity('salon', 'عملية تصنيف', 'تم تسجيل إجراء ضمن القسم بنجاح');
  ```
