...
const generateStaffPage = (title, moduleName, entityName, roles, departmentKey) => `import React, { useState } from 'react';
...
export const ${moduleName} = () => {
  ...
  const staff = useLiveQuery(() => 
    db.users.filter(u => u.department === '${departmentKey}').toArray()
  ) || [];
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center">
              <h2 className="text-xl font-bold text-slate-800">
                {isEdit ? 'تعديل بيانات الموظف' : \`إضافة \${entityName} جديد\`}
              </h2>
              <button 
...
const configs = [
  { moduleFolder: 'school', title: 'المدرسة', moduleName: 'SchoolStaff', entityName: 'موظف/إداري', roles: ['إداري', 'مشرف دور', 'أمن', 'عامل نظافة', 'محاسب', 'أخصائي اجتماعي', 'سائق حافلة'], key: 'school' },
  { moduleFolder: 'garage', title: 'الورشة', moduleName: 'GarageStaff', entityName: 'عامل/فني', roles: ['مدير ورشة', 'مهندس ميكانيكا', 'مهندس كهرباء', 'فني فحص', 'سمكري', 'مساعد فني', 'عامل غسيل'], key: 'garage' },
  { moduleFolder: 'gym', title: 'النادي الرياضي', moduleName: 'GymStaff', entityName: 'موظف', roles: ['مدير فرع', 'كابتن صالة', 'كابتن شخصي', 'موظف استقبال', 'أخصائي تغذية', 'أخصائي علاج طبيعي', 'عامل نظافة'], key: 'gym' },
  { moduleFolder: 'hotel', title: 'الفندق', moduleName: 'HotelStaff', entityName: 'موظف فندق', roles: ['مدير فندق', 'موظف استقبال', 'خدمة غرف (Housekeeping)', 'مدير مطعم/كافيه', 'طاهي', 'موظف أمن', 'حامل حقائب (Bellboy)'], key: 'hotel' },
  { moduleFolder: 'clinics', title: 'العيادات والمجمع الطبي', moduleName: 'ClinicStaff', entityName: 'عضو كادر طبي', roles: ['طبيب استشاري', 'طبيب أخصائي', 'طبيب عام', 'ممرض/ة', 'فني مختبر', 'أخصائي أشعة', 'موظف استقبال', 'مدير طبي'], key: 'clinics' },
  { moduleFolder: 'legal', title: 'المكتب القانوني', moduleName: 'LegalStaff', entityName: 'محامي/إداري', roles: ['محامي شريك', 'محامي متدرب', 'مستشار قانوني', 'سكرتير تنفيذي', 'مندوب محاكم', 'باحث قانوني', 'محاسب'], key: 'legal' },
  { moduleFolder: 'manufacturing', title: 'المصنع', moduleName: 'ManufacturingStaff', entityName: 'عامل مصنع', roles: ['مدير إنتاج', 'مهندس جودة', 'مشرف وردية', 'فني صيانة آلات', 'مشغل ماكينة', 'عامل تعبئة وتغليف', 'أمين مستودع'], key: 'manufacturing' },
  { moduleFolder: 'restaurant', title: 'المطعم', moduleName: 'RestaurantStaff', entityName: 'طاقم المطعم', roles: ['مدير مطعم', 'شيف عمومي (Head Chef)', 'شيف قسم', 'مساعد طباخ', 'كاشير', 'مقدم طعام (Waiter)', 'عامل نظافة', 'مندوب توصيل'], key: 'restaurant' },
  { moduleFolder: 'realestate', title: 'العقارات', moduleName: 'RealEstateStaff', entityName: 'موظف عقاري', roles: ['مدير أملاك', 'مسوق عقاري', 'محصل إيجارات', 'موظف استقبال', 'مشرف صيانة', 'محاسب', 'مندوب قانوني'], key: 'realestate' }
];

for (const config of configs) {
  let dirPath = '';
  // Handle root level pages vs module dir pages based on previous creation script logic
  ...
  if (fs.existsSync(filePath)) {
     fs.writeFileSync(filePath, generateStaffPage(config.title, config.moduleName, config.entityName, config.roles, config.key));
  }
}

console.log('Finished updating staff pages with proper CRUD');
