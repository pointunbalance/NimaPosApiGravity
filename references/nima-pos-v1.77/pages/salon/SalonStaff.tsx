...
export const SalonStaff = () => {
  ...
  const records = useLiveQuery(() => db.salonStaff.toArray()) || [];
  ...
  const handleOpenModal = (editMode = false, item: any = null) => {
    ...
    if (editMode && item) {
      ...
      setCurrentId(null);
      setFormData({
        name: "رومان تيموشوك", // Compliance with Ukrainian Christian names policy
        specialization: "تصفيف وتسريحات شعر متقدمة",
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit && currentId) {
        await db.salonStaff.update(currentId, formData);
        success('تم تحديث بيانات الموظف بالصالون بنجاح');
      } else {
        await db.salonStaff.add(formData);
        success('تم إضافة موظف صالون جديد للهيكل الإداري');
      }
      setIsModalOpen(false);
    } catch (err) {
      console.error(err);
      error('حدث خطأ أثناء حفظ بيانات الموظف');
    }
  };
  ...
  const confirmDelete = async () => {
    if (!deleteId) return;
    try {
      await db.salonStaff.delete(deleteId);
      success('تم حذف سجل الموظف بنجاح من الصالون');
    } catch (err) {
      error('تعذر حذف السجل المحدد');
    }
    setDeleteId(null);
  ...
  return (
    <div className="p-6 space-y-6" dir="rtl">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">إدارة طاقم العمل</h1>
          <p className="text-slate-500 mt-1">إدارة سجلات إدارة طاقم العمل</p>
        </div>
        <button 
        ...
        <div className="p-4 border-b border-slate-200 flex flex-col md:flex-row gap-4 bg-slate-50">
          <div className="relative flex-1">
            ...
            <input 
              ...
              placeholder="بحث في إدارة طاقم العمل..." 
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 border-b border-slate-200 text-right">
              <tr>
                ...
                <th className="px-6 py-4 font-semibold text-slate-600">التخصص</th>
                <th className="px-6 py-4 font-semibold text-slate-600">رقم الجوال</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredRecords.length === 0 ? (
                ...
                <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 font-medium text-slate-800">{item.specialization}</td>
                  <td className="px-6 py-4 font-medium text-slate-800">{item.phone}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSave} className="p-6 space-y-5">
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">رقم الجوال</label>
                <input 
                  type="text" 
      ...
      <ConfirmModal
        ...
        message="هل أنت متأكد من رغبتك في حذف سجل هذا الموظف من الصالون بشكل نهائي؟"
