...
export const ClinicInsurance = () => {
  ...
  const records = useLiveQuery(() => db.clinicInsuranceCompanies.toArray()) || [];
  ...
  const handleOpenModal = (editMode = false, item: any = null) => {
    ...
    if (editMode && item) {
      ...
      setCurrentId(null);
      setFormData({"name":"","contactPerson":"","phone":"","discountRate":"0"});
    }
    setIsModalOpen(true);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit && currentId) {
        await db.clinicInsuranceCompanies.update(currentId, formData);
      } else {
        await db.clinicInsuranceCompanies.add(formData);
      }
  ...
  const handleDelete = async (id: number) => {
    if (confirm('هل أنت متأكد من الحذف؟')) {
      await db.clinicInsuranceCompanies.delete(id);
    }
  };
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">شركات التأمين</h1>
          <p className="text-slate-500">إدارة سجلات شركات التأمين</p>
        </div>
        <button 
          ...
          <table className="w-full">
            <thead className="bg-slate-50 border-b border-slate-200 text-right">
              <tr>
                ...
                <th className="px-6 py-4 text-sm font-semibold text-slate-600">اسم الشركة</th>
                <th className="px-6 py-4 text-sm font-semibold text-slate-600">مسؤول التواصل</th>
                <th className="px-6 py-4 text-sm font-semibold text-slate-600">رقم الهاتف</th>
                <th className="px-6 py-4 text-sm font-semibold text-slate-600">نسبة الخصم / التحمل (%)</th>
            ...
            <tbody className="divide-y divide-slate-200">
              {filteredRecords.length === 0 ? (
                ...
                <tr key={item.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-6 py-4 text-sm text-slate-800">{item.contactPerson}</td>
                  <td className="px-6 py-4 text-sm text-slate-800">{item.phone}</td>
                  <td className="px-6 py-4 text-sm text-slate-800">{item.discountRate}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden">
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">نسبة الخصم / التحمل (%)</label>
                <input 
                  type="number" 
