...
export const SchoolHomework = () => {
  ...
  const records = useLiveQuery(() => db.schoolHomework.toArray()) || [];
  ...
  const handleOpenModal = (editMode = false, item: any = null) => {
    ...
    if (editMode && item) {
      ...
      setCurrentId(null);
      setFormData({"subject":"","grade":"","title":"","dueDate":""});
    }
    setIsModalOpen(true);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit && currentId) {
        await db.schoolHomework.update(currentId, formData);
        success('تم تحديث البيانات بنجاح');
      } else {
        await db.schoolHomework.add(formData);
  ...
  const handleDelete = (id: number) => {
    triggerConfirmation('تأكيد الحذف', 'هل أنت متأكد من الحذف؟', async () => {
      try {
        await db.schoolHomework.delete(id);
  ...
  return (
    <div className="p-6 space-y-6" dir="rtl">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">الواجبات والتكليفات</h1>
          <p className="text-slate-500 mt-1">إدارة سجلات الواجبات والتكليفات</p>
        </div>
        <button 
        ...
        <div className="p-4 border-b border-slate-200 flex flex-col md:flex-row gap-4 bg-slate-50">
          <div className="relative flex-1">
            ...
            <input 
              ...
              placeholder="بحث في الواجبات والتكليفات..." 
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 border-b border-slate-200 text-right">
              <tr>
                ...
                <th className="px-6 py-4 font-semibold text-slate-600">الفصل الدراسي</th>
                <th className="px-6 py-4 font-semibold text-slate-600">عنوان التكليف</th>
                <th className="px-6 py-4 font-semibold text-slate-600">تاريخ التسليم</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredRecords.length === 0 ? (
                ...
                <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 font-medium text-slate-800">{item.grade}</td>
                  <td className="px-6 py-4 font-medium text-slate-800">{item.title}</td>
                  <td className="px-6 py-4 font-medium text-slate-800">{item.dueDate}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSave} className="p-6 space-y-5">
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">عنوان التكليف</label>
                <input 
                  type="text" 
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">تاريخ التسليم</label>
                <input 
                  type="date" 
