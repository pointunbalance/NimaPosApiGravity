...
export const SchoolCertificates = () => {
  ...
  const records = useLiveQuery(() => db.schoolCertificates.toArray()) || [];
  ...
  const handleOpenModal = (editMode = false, item: any = null) => {
    ...
    if (editMode && item) {
      ...
      setCurrentId(null);
      setFormData({"studentName":"","type":"شهادة دراسية","date":"","status":"معتمد"});
    }
    setIsModalOpen(true);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit && currentId) {
        await db.schoolCertificates.update(currentId, formData);
        success('تم تحديث البيانات بنجاح');
      } else {
        await db.schoolCertificates.add(formData);
  ...
  const handleDelete = (id: number) => {
    triggerConfirmation('تأكيد الحذف', 'هل أنت متأكد من الحذف؟', async () => {
      try {
        await db.schoolCertificates.delete(id);
  ...
  return (
    <div className="p-6 space-y-6" dir="rtl">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">الشهادات والإفادات</h1>
          <p className="text-slate-500 mt-1">إدارة سجلات الشهادات والإفادات</p>
        </div>
        <button 
        ...
        <div className="p-4 border-b border-slate-200 flex flex-col md:flex-row gap-4 bg-slate-50">
          <div className="relative flex-1">
            ...
            <input 
              ...
              placeholder="بحث في الشهادات والإفادات..." 
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 border-b border-slate-200 text-right">
              <tr>
                ...
                <th className="px-6 py-4 font-semibold text-slate-600">اسم الطالب</th>
                <th className="px-6 py-4 font-semibold text-slate-600">نوع الوثيقة</th>
                <th className="px-6 py-4 font-semibold text-slate-600">تاريخ الإصدار</th>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSave} className="p-6 space-y-5">
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">اسم الطالب</label>
                <input 
                  type="text" 
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">نوع الوثيقة</label>
                <input 
                  type="text" 
                  value={formData.type}
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">تاريخ الإصدار</label>
                <input 
                  type="date" 
                  value={formData.date}
