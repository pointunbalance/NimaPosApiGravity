...
import { Plus, Search, Filter, MoreHorizontal, Edit2, Trash2, X, CalendarClock } from 'lucide-react';
import { useLiveQuery } from 'dexie-react-hooks';
import { db } from '../../db';

export const GarageAppointments = () => {
  ...
  const records = useLiveQuery(() => db.garageAppointments.toArray()) || [];
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit && currentId) {
        await db.garageAppointments.update(currentId, formData);
      } else {
        await db.garageAppointments.add(formData);
  ...
  const handleDelete = async (id: number) => {
    if (confirm('هل أنت متأكد من الحذف؟')) {
      await db.garageAppointments.delete(id);
    }
  };
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">حجز المواعيد</h1>
          <p className="text-slate-500">إدارة سجلات حجز المواعيد</p>
        </div>
        <button 
        ...
        <div className="p-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
          <div className="relative flex-1 max-w-md">
            ...
            <input 
              ...
              placeholder="بحث في حجز المواعيد..." 
          ...
          <table className="w-full">
            <thead className="bg-slate-50 border-b border-slate-200 text-right">
              <tr>
                ...
                <th className="px-6 py-4 text-sm font-semibold text-slate-600">رقم السيارة</th>
            ...
            <tbody className="divide-y divide-slate-200">
              {filteredRecords.length === 0 ? (
                ...
                <tr key={item.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-6 py-4 text-sm text-slate-800">{item.customerId}</td>
                  <td className="px-6 py-4 text-sm text-slate-800">{item.vehicleId}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden">
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">رقم السيارة</label>
                <input 
                  type="text" 
