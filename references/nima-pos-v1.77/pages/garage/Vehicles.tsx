...
export const Vehicles = () => {
  ...
  const records = useLiveQuery(() => db.garageVehiclesList.toArray()) || [];
  ...
  const handleOpenModal = (editMode = false, item: any = null) => {
    ...
    if (editMode && item) {
      ...
      setCurrentId(null);
      setFormData({"plateNumber":"","make":"","model":"","customerId":""});
    }
    setIsModalOpen(true);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit && currentId) {
        await db.garageVehiclesList.update(currentId, formData);
      } else {
        await db.garageVehiclesList.add(formData);
      }
  ...
  const handleDelete = async (id: number) => {
    if (confirm('هل أنت متأكد من الحذف؟')) {
      await db.garageVehiclesList.delete(id);
    }
  };
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">سجل المركبات</h1>
          <p className="text-slate-500">إدارة سجلات سجل المركبات</p>
        </div>
        <button 
        ...
        <div className="p-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
          <div className="relative flex-1 max-w-md">
            ...
            <input 
              ...
              placeholder="بحث في سجل المركبات..." 
            ...
            <tbody className="divide-y divide-slate-200">
              {filteredRecords.length === 0 ? (
                ...
                <tr key={item.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-6 py-4 text-sm text-slate-800">{item.plateNumber}</td>
                  <td className="px-6 py-4 text-sm text-slate-800">{item.make}</td>
                  <td className="px-6 py-4 text-sm text-slate-800">{item.model}</td>
