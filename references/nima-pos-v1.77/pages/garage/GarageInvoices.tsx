...
export const GarageInvoices = () => {
  ...
  const records = useLiveQuery(() => db.garageInvoices.toArray()) || [];
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit && currentId) {
        await db.garageInvoices.update(currentId, formData);
      } else {
        await db.garageInvoices.add(formData);
        ...
        if (formData.status === 'paid' && Number(formData.amount) > 0) {
          ...
          if (cashAcc && revenueAcc) {
            await AccountingEngine.postEntry({
              ...
              description: `تسوية مالية (${formData.status}) - قسم GarageInvoices`,
              lines: [
  ...
  const handleDelete = async (id: number) => {
    if (confirm('هل أنت متأكد من الحذف؟')) {
      await db.garageInvoices.delete(id);
    }
  };
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">فواتير الصيانة</h1>
          <p className="text-slate-500">إدارة سجلات فواتير الصيانة</p>
        </div>
        <button 
        ...
        <div className="p-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
          <div className="relative flex-1 max-w-md">
            ...
            <input 
              ...
              placeholder="بحث في فواتير الصيانة..." 
