...
interface BranchTransfersHeaderProps {
  searchTerm: string;
  setSearchTerm: (term: string) => void;
  statusFilter: 'all' | 'pending' | 'in_transit' | 'completed' | 'cancelled';
  setStatusFilter: (status: 'all' | 'pending' | 'in_transit' | 'completed' | 'cancelled') => void;
  onNewTransferClick: () => void;
}

const BranchTransfersHeader: React.FC<BranchTransfersHeaderProps> = ({
  searchTerm,
  setSearchTerm,
  statusFilter,
  setStatusFilter,
    ...
    <div className="bg-white border-b border-slate-200 px-8 py-6 shrink-0">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-black text-slate-900 flex items-center gap-3 tracking-tight">
            ...
            تحويلات الفروع والمخازن
          </h1>
          <p className="text-slate-500 font-medium mt-2">إدارة ومتابعة نقل البضائع بين الفروع والمخازن</p>
        </div>
        <button
      ...
      <div className="mt-8 flex flex-col md:flex-row gap-4 items-center bg-slate-50 p-3 rounded-2xl border border-slate-100">
        <div className="relative flex-1 w-full">
          ...
          <input
            type="text"
            placeholder="بحث برقم التحويل أو اسم المخزن..."
            value={searchTerm}
...
export default BranchTransfersHeader;
