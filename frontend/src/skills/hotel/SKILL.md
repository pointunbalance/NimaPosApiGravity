---
name: "hotel"
description: "Guidelines and functional requirements for الفنادق والضيافة"
---

# الفنادق والضيافة

هذا الملف (Skill) يوفر معايير هندسية متكاملة لبناء وتوسيع قسم **الفنادق والضيافة** داخل التطبيق، معتمداً على بيئة العمل الحالية (React + Dexie.js + TypeScript) لضمان العمل بدون اتصال بالإنترنت (Offline-First).
...
- **لوحة قيادة الفندق** (/hotel/dashboard)
...
- **سجل العمليات (Audit Logging)**: توثيق الحركات الحساسة (إضافة، تعديل، حذف) عبر دالة `logActivity()` كما في هذا المثال:
  ...
  await logActivity('hotel', 'عملية تصنيف', 'تم تسجيل إجراء ضمن القسم بنجاح');
  ```
