---
name: "products"
description: "Guidelines and functional requirements for إدارة المنتجات"
---

# إدارة المنتجات

هذا الملف (Skill) يوفر معايير هندسية متكاملة لبناء وتوسيع قسم **إدارة المنتجات** داخل التطبيق، معتمداً على بيئة العمل الحالية (React + Dexie.js + TypeScript) لضمان العمل بدون اتصال بالإنترنت (Offline-First).
...
- **سجل العمليات (Audit Logging)**: توثيق الحركات الحساسة (إضافة، تعديل، حذف) عبر دالة `logActivity()` كما في هذا المثال:
  ...
  await logActivity('products', 'عملية تصنيف', 'تم تسجيل إجراء ضمن القسم بنجاح');
  ```
