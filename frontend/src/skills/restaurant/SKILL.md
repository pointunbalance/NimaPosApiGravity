---
name: "restaurant"
description: "Guidelines and functional requirements for المطعم والكافيه"
---

# المطعم والكافيه

هذا الملف (Skill) يوفر معايير هندسية متكاملة لبناء وتوسيع قسم **المطعم والكافيه** داخل التطبيق، معتمداً على بيئة العمل الحالية (React + Dexie.js + TypeScript) لضمان العمل بدون اتصال بالإنترنت (Offline-First).
...
- **لوحة التحكم** (/restaurant/dashboard)
...
- **سجل العمليات (Audit Logging)**: توثيق الحركات الحساسة (إضافة، تعديل، حذف) عبر دالة `logActivity()` كما في هذا المثال:
  ...
  await logActivity('restaurant', 'عملية تصنيف', 'تم تسجيل إجراء ضمن القسم بنجاح');
  ```
