---
name: "taxes_closing"
description: "Guidelines and functional requirements for الضرائب والإقفال"
---

# الضرائب والإقفال

هذا الملف (Skill) يوفر معايير هندسية متكاملة لبناء وتوسيع قسم **الضرائب والإقفال** داخل التطبيق، معتمداً على بيئة العمل الحالية (React + Dexie.js + TypeScript) لضمان العمل بدون اتصال بالإنترنت (Offline-First).
...
- **سجل العمليات (Audit Logging)**: توثيق الحركات الحساسة (إضافة، تعديل، حذف) عبر دالة `logActivity()` كما في هذا المثال:
  ...
  await logActivity('taxes_closing', 'عملية تصنيف', 'تم تسجيل إجراء ضمن القسم بنجاح');
  ```
