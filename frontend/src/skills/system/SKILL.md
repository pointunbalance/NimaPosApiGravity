---
name: "system"
description: "Guidelines and functional requirements for إدارة النظام والأمان"
---

# إدارة النظام والأمان

هذا الملف (Skill) يوفر معايير هندسية متكاملة لبناء وتوسيع قسم **إدارة النظام والأمان** داخل التطبيق، معتمداً على بيئة العمل الحالية (React + Dexie.js + TypeScript) لضمان العمل بدون اتصال بالإنترنت (Offline-First).
...
- **المركز السحابي للفرع والملاك** (/owner-cloud)
...
- **سجل العمليات (Audit Logging)**: توثيق الحركات الحساسة (إضافة، تعديل، حذف) عبر دالة `logActivity()` كما في هذا المثال:
  ...
  await logActivity('system', 'عملية تصنيف', 'تم تسجيل إجراء ضمن القسم بنجاح');
  ```
