...
description: "Guidelines and functional requirements for العمليات المالية المتقدمة"
---

# العمليات المالية المتقدمة

هذا الملف (Skill) يوفر معايير هندسية متكاملة لبناء وتوسيع قسم **العمليات المالية المتقدمة** داخل التطبيق، معتمداً على بيئة العمل الحالية (React + Dexie.js + TypeScript) لضمان العمل بدون اتصال بالإنترنت (Offline-First).
...
- **الخزينة والسيولة** (/accounting/treasury)
...
- **سجل العمليات (Audit Logging)**: توثيق الحركات الحساسة (إضافة، تعديل، حذف) عبر دالة `logActivity()` كما في هذا المثال:
  ...
  await logActivity('advanced_accounting', 'عملية تصنيف', 'تم تسجيل إجراء ضمن القسم بنجاح');
  ```
