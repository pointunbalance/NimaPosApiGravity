---
name: "clinics"
description: "Guidelines and functional requirements for العيادات والمراكز الطبية"
---

# العيادات والمراكز الطبية

هذا الملف (Skill) يوفر معايير هندسية متكاملة لبناء وتوسيع قسم **العيادات والمراكز الطبية** داخل التطبيق، معتمداً على بيئة العمل الحالية (React + Dexie.js + TypeScript) لضمان العمل بدون اتصال بالإنترنت (Offline-First).
...
- **لوحة قيادة العيادة** (/clinics/dashboard)
...
- **سجل العمليات (Audit Logging)**: توثيق الحركات الحساسة (إضافة، تعديل، حذف) عبر دالة `logActivity()` كما في هذا المثال:
  ...
  await logActivity('clinics', 'عملية تصنيف', 'تم تسجيل إجراء ضمن القسم بنجاح');
  ```
