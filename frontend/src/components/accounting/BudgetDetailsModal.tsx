...
const BudgetDetailsModal: React.FC<BudgetDetailsModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 print:p-0 print:bg-white">
      <div className="bg-white rounded-3xl w-full max-w-5xl max-h-[90vh] flex flex-col shadow-2xl animate-in zoom-in-95 print:shadow-none print:max-h-none print:rounded-none">
        <div className="flex justify-between items-center p-6 border-b border-slate-100 print:hidden">
          <div>
            <h3 className="font-bold text-xl text-slate-800">تفاصيل الموازنة: {budget.name}</h3>
            <p className="text-sm text-slate-500">السنة المالية: {fiscalYear?.name}</p>
          </div>
          <div className="flex gap-2">
            <button onClick={() => window.print()} className="p-2 hover:bg-slate-100 rounded-full text-slate-500 transition-colors">
        ...
        <div className="p-6 overflow-y-auto flex-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 print:border-black print:bg-white">
                    <p className="text-sm font-bold text-slate-500 mb-1">إجمالي الموازنة</p>
                    <p className="text-2xl font-black text-slate-800">{formatCurrency(totalBudget)}</p>
                </div>
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 print:border-black print:bg-white">
                    <p className="text-sm font-bold text-slate-500 mb-1">الفعلي</p>
                    <p className="text-2xl font-black text-slate-800">{formatCurrency(totalActual)}</p>
                </div>
                <div className={`p-4 rounded-2xl border print:border-black print:bg-white ${totalVariance < 0 ? 'bg-red-50 border-red-100' : 'bg-emerald-50 border-emerald-100'}`}>
                    <p className="text-sm font-bold mb-1 flex items-center gap-1">
                        الانحراف (المتبقي)
                    </p>
                    <p className={`text-2xl font-black ${totalVariance < 0 ? 'text-red-600' : 'text-emerald-600'}`}>
                        {formatCurrency(totalVariance)}
                    </p>
                </div>
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 print:border-black print:bg-white">
                    <p className="text-sm font-bold text-slate-500 mb-1">نسبة الاستهلاك</p>
                    <div className="flex items-end gap-2">
                        <p className="text-2xl font-black text-slate-800">{totalPercent.toFixed(1)}%</p>
                        <div className="flex-1 h-2 bg-slate-200 rounded-full mb-2 overflow-hidden print:border print:border-black">
                            <div 
            ...
            <div className="border border-slate-200 rounded-xl overflow-hidden print:border-black">
                <table className="w-full text-right text-sm">
                    <thead className="bg-slate-50 text-slate-500 font-bold print:bg-white print:text-black border-b border-slate-200 print:border-black">
                        <tr>
                        ...
                        {budget.lines.map((line, idx) => {
                            ...
                            return (
                                <tr key={idx} className="hover:bg-slate-50 transition-colors">
                                    ...
                                    <td className="p-4 font-bold text-slate-800 print:text-black">
                                        {formatCurrency(line.amount)}
                                    </td>
                                    <td className="p-4 font-bold text-slate-800 print:text-black">
                                        {formatCurrency(actual)}
                                    </td>
                                    <td className={`p-4 font-bold ${isOver ? 'text-red-600' : 'text-emerald-600'} print:text-black`}>
                                    ...
                                    <td className="p-4">
                                        <div className="flex items-center gap-2">
                                            <span className="font-bold w-12">{percent.toFixed(1)}%</span>
                                            <div className="w-24 h-2 bg-slate-100 rounded-full overflow-hidden print:border print:border-black">
                                                <div 
                    ...
                    <tfoot className="bg-slate-50 font-bold text-slate-800 print:bg-white print:text-black border-t-2 border-slate-200 print:border-black">
                        <tr>
                            ...
                            <td className={`p-4 ${totalVariance < 0 ? 'text-red-600' : 'text-emerald-600'} print:text-black`}>
                                <div className="flex items-center gap-1">
