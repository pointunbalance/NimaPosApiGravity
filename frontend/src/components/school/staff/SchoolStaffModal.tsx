...
export const SchoolStaffModal = (props: any) => {
   ...
   return (
       <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
           <div className="bg-white rounded-3xl shadow-xl w-full max-w-lg overflow-hidden animate-in zoom-in duration-200">
               ...
               <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-6 max-h-[80vh] overflow-y-auto w-full">
                   <div>
                       <label className="block text-sm font-bold text-slate-700 mb-2">اسم الموظف الموظف بالكامل *</label>
                   ...
                   <div className="grid grid-cols-2 gap-4">
                       <div>
                           ...
                           <input type="number" value={formData.salary || 0} onChange={(e) => setFormData({...formData, salary: Number(e.target.value)})} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none" required />
                       </div>
                       <div>
                   ...
                   <div>
                       ...
                       <input type="number" min="2000" value={formData.yearJoined || new Date().getFullYear()} onChange={(e) => setFormData({...formData, yearJoined: Number(e.target.value)})} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none" />
                   </div>
