...
export const ManufacturingStaff = () => {
  const rolesList = ["مدير إنتاج","مهندس جودة","مشرف وردية","فني صيانة آلات","مشغل ماكينة","عامل تعبئة وتغليف","أمين مستودع"];
  ...
  const staff = useLiveQuery(() => 
    db.users.filter(u => u.department === 'manufacturing').toArray()
  ) || [];
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">إدارة موظفي المصنع</h1>
          <p className="text-slate-500">سجل وحسابات رواتب وأداء طاقم المصنع</p>
        </div>
        <button 
           onClick={() => handleOpenModal(false)}
           className="bg-indigo-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-indigo-700">
          <Plus className="w-5 h-5" />
          <span>إضافة عامل مصنع جديد</span>
        </button>
      </div>
