...
export const Jobs = () => {
  ...
  const records = useLiveQuery(() => db.garageJobsList.toArray()) || [];
  ...
  const handleOpenModal = (editMode = false, item: any = null) => {
    ...
    if (editMode && item) {
      ...
      setCurrentId(null);
      setFormData({"vehicleId":"","description":"","estimatedCost":"0","status":"قيد الانتظار"});
    }
    setIsModalOpen(true);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit && currentId) {
        await db.garageJobsList.update(currentId, formData);
      } else {
        await db.garageJobsList.add(formData);
      }
  ...
  const handleDelete = async (id: number) => {
    if (confirm('هل أنت متأكد من الحذف؟')) {
      await db.garageJobsList.delete(id);
    }
  };
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">أوامر الشغل</h1>
          <p className="text-slate-500">إدارة سجلات أوامر الشغل</p>
        </div>
        <button 
          ...
          <table className="w-full">
            <thead className="bg-slate-50 border-b border-slate-200 text-right">
              <tr>
                ...
                <th className="px-6 py-4 text-sm font-semibold text-slate-600">رقم اللوحة</th>
                <th className="px-6 py-4 text-sm font-semibold text-slate-600">وصف العطل</th>
                <th className="px-6 py-4 text-sm font-semibold text-slate-600">التكلفة التقديرية</th>
            ...
            <tbody className="divide-y divide-slate-200">
              {filteredRecords.length === 0 ? (
                ...
                <tr key={item.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-6 py-4 text-sm text-slate-800">{item.estimatedCost}</td>
