...
export const Technicians = () => {
  ...
  const records = useLiveQuery(() => db.garageTechniciansList.toArray()) || [];
  ...
  const handleOpenModal = (editMode = false, item: any = null) => {
    ...
    if (editMode && item) {
      ...
      setCurrentId(null);
      setFormData({"name":"","specialization":"","phone":"","status":"متاح"});
    }
    setIsModalOpen(true);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit && currentId) {
        await db.garageTechniciansList.update(currentId, formData);
      } else {
        await db.garageTechniciansList.add(formData);
      }
  ...
  const handleDelete = async (id: number) => {
    if (confirm('هل أنت متأكد من الحذف؟')) {
      await db.garageTechniciansList.delete(id);
    }
  };
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">الفنيون</h1>
          <p className="text-slate-500">إدارة سجلات الفنيون</p>
        </div>
        <button 
          ...
          <table className="w-full">
            <thead className="bg-slate-50 border-b border-slate-200 text-right">
              <tr>
                ...
                <th className="px-6 py-4 text-sm font-semibold text-slate-600">اسم الفني</th>
                <th className="px-6 py-4 text-sm font-semibold text-slate-600">التخصص</th>
            ...
            <tbody className="divide-y divide-slate-200">
              {filteredRecords.length === 0 ? (
                ...
                <tr key={item.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-6 py-4 text-sm text-slate-800">{item.specialization}</td>
