...
import { Plus, Search, Filter, MoreHorizontal, Edit2, Trash2, X, Settings } from 'lucide-react';
...
export const SpareParts = () => {
  ...
  const records = useLiveQuery(() => db.garageSpareParts.toArray()) || [];
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit && currentId) {
        await db.garageSpareParts.update(currentId, formData);
      } else {
        await db.garageSpareParts.add(formData);
  ...
  const handleDelete = async (id: number) => {
    if (confirm('هل أنت متأكد من الحذف؟')) {
      await db.garageSpareParts.delete(id);
    }
  };
    ...
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500">إدارة سجلات قطع الغيار</p>
        </div>
        <button 
          ...
          <table className="w-full">
            <thead className="bg-slate-50 border-b border-slate-200 text-right">
              <tr>
                ...
                <th className="px-6 py-4 text-sm font-semibold text-slate-600">رقم القطعة</th>
            ...
            <tbody className="divide-y divide-slate-200">
              {filteredRecords.length === 0 ? (
                ...
                <tr key={item.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-6 py-4 text-sm text-slate-800">{item.partNumber}</td>
                  <td className="px-6 py-4 text-sm text-slate-800">{item.price}</td>
                  <td className="px-6 py-4 text-sm text-slate-800">{item.stock}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden">
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">رقم القطعة</label>
                <input 
                  type="text" 
