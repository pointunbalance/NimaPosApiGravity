...
export const ClinicStaff = () => {
  const rolesList = ["طبيب استشاري","طبيب أخصائي","طبيب عام","ممرض/ة","فني مختبر","أخصائي أشعة","موظف استقبال","مدير طبي"];
  ...
  const staff = useLiveQuery(() => 
    db.users.filter(u => u.department === 'clinics').toArray()
  ) || [];
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">إدارة موظفي العيادات والمجمع الطبي</h1>
          <p className="text-slate-500">سجل وحسابات رواتب وأداء طاقم العيادات والمجمع الطبي</p>
        </div>
        <button 
           onClick={() => handleOpenModal(false)}
           className="bg-indigo-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-indigo-700">
          <Plus className="w-5 h-5" />
          <span>إضافة عضو كادر طبي جديد</span>
        </button>
      </div>
