...
export const Teachers = () => {
  ...
  const records = useLiveQuery(() => db.schoolTeachers.toArray()) || [];
  ...
  const handleOpenModal = (editMode = false, item: any = null) => {
    ...
    if (editMode && item) {
      ...
      setCurrentId(null);
      setFormData({"name":"","subject":"","phone":"","status":"نشط"});
    }
    setIsModalOpen(true);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit && currentId) {
        await db.schoolTeachers.update(currentId, formData);
        success('تم تحديث البيانات بنجاح');
      } else {
        await db.schoolTeachers.add(formData);
  ...
  const handleDelete = (id: number) => {
    triggerConfirmation('تأكيد الحذف', 'هل أنت متأكد من الحذف؟', async () => {
      try {
        await db.schoolTeachers.delete(id);
    ...
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500">إدارة سجلات المعلمون</p>
        </div>
        <button 
          ...
          <table className="w-full">
            <thead className="bg-slate-50 border-b border-slate-200 text-right">
              <tr>
                ...
                <th className="px-6 py-4 text-sm font-semibold text-slate-600">اسم المعلم</th>
            ...
            <tbody className="divide-y divide-slate-200">
              {filteredRecords.length === 0 ? (
                ...
                <tr key={item.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-6 py-4 text-sm text-slate-800">{item.subject}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden">
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">اسم المعلم</label>
                <input 
                  type="text" 
                  value={formData.name}
